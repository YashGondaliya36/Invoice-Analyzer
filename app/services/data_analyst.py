...
class DataAnalystService:
    ...
    def _execute_code(self, code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            charts_file = self.file_handler.get_visualization_file()
            has_viz = charts_file.exists()
            
            return {
                "success": True,
                "data": str(result_data) if result_data is not None else None,
                "visualization": str(charts_file.name) if has_viz else None # Return filename only (frontend fetches via URL)
            }
        except Exception as e:
            return {
                "success": False,
